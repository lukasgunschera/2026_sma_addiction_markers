...
## ======================================================================================================================= ##
## Authors:   Clelia Veith, based on Mehrhof & Nord (2024)
...
task_plot <- function(task_dat, 
                      main_title = "Acceptance proportion by effort and reward 
                      level",
  ...
  # Per-subject per-condition means
  choiceMean_subj <- aggregate(choice ~ subjID + effort_a + amount_a, 
                               FUN = function(x) mean(x) * 100, 
                               dat = task_dat)
  
  # Overall condition means (for plotting)
  choiceMean <- aggregate(choice ~ effort_a + amount_a, 
                          FUN = mean, 
                          dat = choiceMean_subj)
  
  # Cousineau-Morey within-subject SE
  subj_means <- aggregate(choice ~ subjID, FUN = mean, dat = choiceMean_subj)
  grand_mean <- mean(choiceMean_subj$choice)
  
  choiceMean_subj <- merge(choiceMean_subj, subj_means, by = "subjID", 
                           suffixes = c("", "_subj"))
  choiceMean_subj$choice_norm <- choiceMean_subj$choice - 
    choiceMean_subj$choice_subj + grand_mean
  
  k <- length(unique(choiceMean_subj$effort_a)) * 
    length(unique(choiceMean_subj$amount_a))
  morey_correction <- sqrt(k / (k - 1))
  
  choiceSE <- aggregate(choice_norm ~ effort_a + amount_a,
                        FUN = function(x) (sd(x) / sqrt(length(x))) * morey_correction,
                        dat = choiceMean_subj)
  ...
  if(direction == "effort_by_reward"){
    ...
    if(plot_type == "bar_plot"){
      for(i in 1:4){
        choicePlot[[i]] <- ggplot(data=choicesPlotDat[choicesPlotDat$amount_a 
                                                      == rewards[i],], 
      ...
      choicePlot <- ggplot(data=choicesPlotDat, 
        ...
        geom_point(aes(color = amount_a, shape = amount_a), 
                   position = position_dodge(0.2), size = 1.5) +
        geom_line(aes(color = amount_a), position = 
                    position_dodge(0.2), size = 0.75) +
    ...
    for(i in 1:4){
      ...
      choicePlot[[i]] <- ggplot(data=choicesPlotDat[choicesPlotDat$effort_a 
                                                    == efforts[i],], 
  ...
  if(arrange_cols != 0 | arrange_rows != 0){
    ...
    choicePlot <- annotate_figure(choicePlot, 
                                  top = text_grob(main_title, 
                                                  face = "bold", size = 18),
                                  left = text_grob("% Accepted (SE)", 
                                                   size = 10, rot = 90),
                                  bottom = text_grob("Effort level",
                                                     size = 10)
    )    
  }
  
  return(choicePlot + theme_classic())
} 
...
raincloud_plot <- function(dat, title, xlab, ylab,
  ...
  if(include_grouping == FALSE){
    rain_plot <- ggplot(data = dat, 
                        aes_string(y = outcome_var, x = predictor_var, 
                                   fill = predictor_var)) + 
...
ppc_plots <- function(ppc_dat, 
                      group_plot = TRUE,
                      indiv_plot = TRUE, 
                      group_plot_title = "Observed vs. model predicted 
                      acceptance proportions",
                      indiv_plot_title = "Subject wise observed vs. 
                      model predicted acceptance proportions "){
  ...
  if(group_plot == TRUE){
    ...
    group_plot_dat <- cbind(group_plot_dat, "group" = 
                              paste(rep(c("pred", "real"), each = 16),
    ...
    for(i in 1:4){
      group_plot[[i]] <- ggplot(
        group_plot_dat[group_plot_dat$amount_a == amount[i],], 
        aes(x=effort_a, y=mean, fill=group)) +
        ...
        scale_fill_manual(values=c('#f29a8c', '#aecdea','#add293', '#ffdf80',
                          ...
                          labels = c("Predicted", "Predicted", "Predicted", 
  ...
  if(indiv_plot == TRUE){
    ...
    R_squared_effort <- round(cor(indiv_plot_effort_dat$observation, 
                                  indiv_plot_effort_dat$prediction_mean)^2, 2)
    
    indiv_plot_effort <- ggplot(indiv_plot_effort_dat, 
                                aes(x = observation, 
                                    y = prediction_mean, 
                                    color = effort_a)) +
      geom_point(size = 2, alpha = 0.25) +
      geom_errorbar(aes(ymin=prediction_hdi_lower, ymax=prediction_hdi_higher), 
                    width=.025, alpha=0.1) +
      scale_color_manual(values=c('#E94D36', '#5B9BD5', '#71AB48', '#FFBF00'), 
                         labels=1:4) + 
      xlim(0,1) + ylim(0,1) +
    ...
    R_squared_effort <- round(cor(indiv_plot_reward_dat$observation, 
                                  indiv_plot_reward_dat$prediction_mean)^2, 2)
    
    indiv_plot_reward <- ggplot(indiv_plot_reward_dat, aes(x=observation, 
                                                           y=prediction_mean, 
                                                           color=amount_a)) +
      geom_point(size=2, alpha=0.25) +
      geom_errorbar(aes(ymin=prediction_hdi_lower, ymax=prediction_hdi_higher), 
